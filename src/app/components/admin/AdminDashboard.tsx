...
export function AdminDashboard() {
  ...
  const [pendingApptCount, setPendingApptCount] = useState<number>(0);
  const [recentPatients, setRecentPatients] = useState<PatientRow[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchStats() {
      const [patients, unread, team, gallery, pendingAppt, recent] = await Promise.all([
      ...
      setPendingApptCount(pendingAppt.count ?? 0);
  ...
  const stats = [
    ...
    { label: "RDV en attente",       value: pendingApptCount, icon: CalendarCheck, color: "bg-amber-500",   trend: pendingApptCount > 0 ? "À valider" : "À jour" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link to="/admin/appointments" className="p-4 border-2 border-brand-600 text-brand-600 rounded-lg hover:bg-brand-50 transition-all text-center">
              <CalendarCheck className="mx-auto mb-2" size={32} />
              <span className="block font-semibold text-sm">Rendez-vous</span>
            </Link>
            <Link to="/admin/calendar" className="p-4 border-2 border-brand-600 text-brand-600 rounded-lg hover:bg-brand-50 transition-all text-center">
              <Calendar className="mx-auto mb-2" size={32} />
              <span className="block font-semibold text-sm">Calendrier</span>
            </Link>
            <Link to="/admin/patients" className="p-4 border-2 border-teal-600 text-teal-600 rounded-lg hover:bg-teal-50 transition-all text-center">
              <Users className="mx-auto mb-2" size={32} />
              <span className="block font-semibold text-sm">Patients</span>
            </Link>
            <Link to="/admin/messages" className="p-4 border-2 border-orange-600 text-orange-600 rounded-lg hover:bg-orange-50 transition-all text-center">
              <MessageSquare className="mx-auto mb-2" size={32} />
              <span className="block font-semibold text-sm">Messages</span>
            </Link>
            <Link to="/admin/content" className="p-4 border-2 border-purple-600 text-purple-600 rounded-lg hover:bg-purple-50 transition-all text-center">
